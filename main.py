...
def main():
    ...
    amazon_thread = threading.Thread(target=scrape_amazon, args=(amazon_url, result_dict))
    flipkart_thread = threading.Thread(target=scrape_flipkart, args=(flipkart_url, result_dict))
    ...
    pred = Prediction(amazon_df, flipkart_df)
    pred.predict()

    summ = Summary(amazon_df, flipkart_df)
    summ.summarize()
